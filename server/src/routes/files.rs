...
use crate::database::db::{FileStruct, InsertableFile, ResponseFile};
...
pub async fn store(data: Form<DataStruct<'_>>) -> ApiResponse {
    ...
    for (i, f) in data.files.iter().enumerate() {
        ...
        let inserted = filesDB
            .insert_one(to_document.to_owned(), None)
            .await
            .unwrap();

        //find the just inserted data
        let new_data = filesDB
            .find_one(Some(doc! { "_id":  inserted.inserted_id.clone() }), None)
            .await
            .unwrap()
...
pub async fn update(id: String, data: Form<DataStruct<'_>>) -> ApiResponse {
    ...
    //get the file from the response body
    let getFile = NamedFile::open(data.files[0].path().unwrap())
        .await
        .unwrap();
    let mut file = getFile.take_file();
    ...
    let to_update: FileStruct = bson::from_bson(Bson::Document(
        filesDB
            .find_one(
    ...
    ApiResponse::ok(json!(ResponseFile::from_file(
        bson::from_bson::<FileStruct>(Bson::Document(to_document.to_owned())).unwrap()
    )))
}
